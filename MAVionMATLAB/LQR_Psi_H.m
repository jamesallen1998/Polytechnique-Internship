...
[z,p,k] = zpkdata(sysCancel);
sysZeros = z{:,1}; % Access Poles from first transfer function for testing 
sysPoles = p{:,1}; % Access Poles from first transfer function for testing
...
%% Optimal Control Design (LQR): H Matrix
...
size(sys_lat)               % SS model with 5 outputs, 2 inputs, 5 states
Q = diag([1.5 0 0 0 15]);   % Q acts on state vector (nxn) -> R(5x5)
R = diag([0.0001 150]);    % R acts on input vector (mxm) -> R(2x2)

K = lqr(Alat,Blat,Q,R);     % Generate optimal gain matrix (mxn) -> R(2x5)
damp(Alat-Blat*K);          % Poles of the closed loop system

co=ctrb(Alat,Blat);         % Controllability
ob=obsv(Alat,Clat);         % Observability
...
% H Matrix: Feedforward gains
H = (-(E-F*K)*(Alat-Blat*K)^-1 *Blat + F)^-1;
...
% Closed Loop SS
sys_cl = ss(Alat-Blat*K,Blat*H,[E-F*K;-K],0,'inputname',inputs,...
    'outputname',outputs);        
size(sys_cl)                     % Size of cl SS model
